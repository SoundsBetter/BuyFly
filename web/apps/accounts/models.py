...
class Supervisor(models.Model):
    user = OneToOneField(
        ...
        related_name=GROUP_SUPERVISORS,
    )
...
class CheckInManager(models.Model):
    user = models.OneToOneField(
        User,
        on_delete=models.CASCADE,
        related_name=GROUP_CHECK_IN_MANAGERS,
    )
...
class GateManager(models.Model):
    user = models.OneToOneField(
        User,
        on_delete=models.CASCADE,
        related_name=GROUP_GATE_MANAGERS,
    )

    def save(self, *args, **kwargs):
        super().save(*args, **kwargs)
        gate_managers, _ = Group.objects.get_or_create(name="gate_managers")
